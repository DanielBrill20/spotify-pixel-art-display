...
<img src="assets/flower_boy_cover.jpg" alt="Flower Boy album cover on LED matrix" height="200"> <img src="assets/game_of_life.jpg" alt="Conway's Game of Life on LED Matrix" height="200"> <img src="assets/not_all_heroes_wear_capes_cover.jpg" alt="NOT ALL HEROES WEAR CAPES album cover on LED matrix" height="200">
...
4.	Every 0.5 seconds, poll the Spotify Web API to fetch the user's currently playing song or episode.
...
4. Apply component patches so the bundled libraries build correctly:
   ...
   ```
5. Select the correct silicon target for your board:
   ```bash
   idf.py set-target esp32s3      # choose esp32, esp32s3, esp32s2, etc.
   ```
6. The Arduino component will cause a build error. The FreeRTOS tick rate needs to change from 100Hz to 1000Hz. While this can be done in menuconfig, it will cause a build error before `idf.py menuconfig` can even open.
      - Open `sdkconfig` and manually change `CONFIG_FREERTOS_HZ=100` to `CONFIG_FREERTOS_HZ=1000`.
         - If `sdkconfig` does not yet exist (but it should if step 5 was successful), you need to prompt one to be generated. Try running `idf.py menuconfig` or `idf.py build`. These commands will fail because of the tick rate issue, but they will prompt `sdkconfig` creation.
...
### Hardware Images
<img src="assets/panel_back.jpg" alt="Back of LED panel" width="280"> <img src="assets/esp32_pinout.png" alt="ESP32 pinout" width="420">
<img src="assets/panel_power_wires.png" alt="Wires to power panel and ESP" width="350"> <img src="assets/hardware_construction.jpg" alt="Panel with all wires connected" width="350">
...
For songs and podcast episodes, the Spotify Web API typically includes the three album cover images of various sizes. The typical output looks something like:
```
"images": [
  {
    "height": 640,
